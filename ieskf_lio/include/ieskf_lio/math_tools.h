...
inline Eigen::Matrix3d Jright(const Eigen::Quaternion<T> &qua)
{
    ...
    mat = Eigen::Matrix<T, 3, 3>::Identity() - (1 - cos(theta_norm)) / (theta_norm * theta_norm + 1e-10) * hat(rotation_vec) + (theta_norm - sin(theta_norm)) / (theta_norm * theta_norm * theta_norm + 1e-10) * hat(rotation_vec) * hat(rotation_vec);
    return mat;
}
