...
{
  velocity_max_ = kmph2mps(conf->velocity_max);//20
  velocity_min_ = kmph2mps(conf->velocity_min);//4
  accel_limit_ = conf->accel_limit;//0.5
  decel_limit_ = conf->decel_limit;//0.3
  r_th_ = conf->radius_thresh;//20
  r_min_ = conf->radius_min;//6
  lookup_crv_width_ = 5;//
  resample_mode_ = conf->resample_mode;//true
  resample_interval_ = conf->resample_interval;//1.0
  velocity_offset_ = conf->velocity_offset;//4
  end_point_offset_ = conf->end_point_offset;//1
  r_inf_ = 10 * r_th_;
...
void VelocityReplanner::resampleLaneWaypoint(const double resample_interval, autoware_msgs::lane* lane)
{
  ...
  //autoware_msgs::lane -> CenterWay3D
