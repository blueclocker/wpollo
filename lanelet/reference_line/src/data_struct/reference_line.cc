...
#include "data_struct/reference_line.h"
...
bool ReferenceLine::IsOnLane(const SLBoundary& sl_boundary) const { // 检查slboundary表示的一个物体是否在当前车道上。
  if (sl_boundary.end_s() < 0 || sl_boundary.start_s() > Length()) { // 如果在后面或者在前面远处
    return false;
  }
...
bool ReferenceLine::GetSLBoundary(const common::math::Box2d& box,
  ...
  for (size_t i = 0; i < corners.size(); ++i) {
    ...
    const auto& p0 = corners[index0]; // 当前corner
    const auto& p1 = corners[index1]; // 下个corner。如果当前是最后一个，那这就是第一个。

    const auto p_mid = (p0 + p1) * 0.5;
    SLPoint sl_point_mid;
    if (!XYToSL(p_mid, &sl_point_mid)) { // 当前corner和下个corner的中点投影到sl坐标系。
    ...
    // sl point的结构就是一个s，一个l。 这里把当前corner插入到sl_boundary中
