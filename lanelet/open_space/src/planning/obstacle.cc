...
void Obstacle::BuildReferenceLineStBoundary(const ReferenceLine& reference_line,
  ...
  // 注意运行这个函数的时候，障碍物已经添加到了某条车道线的path_decision_里面了。也就是说障碍物是附属于某条车道线的。
  // 这个函数作用就是计算该障碍物在其附属的参考线的st图上的占据。
  const auto& adc_param =
      VehicleConfigHelper::Instance()->GetConfig().vehicle_param();
  const double adc_width = adc_param.width();
  if (is_static_ || trajectory_.trajectory_point().empty()) { // trajectory_是障碍物预测轨迹。
    ...
    if (!reference_line.IsBlockRoad(perception_bounding_box_, adc_width)) { // 如果该车道没有被本障碍物堵住，返回。
      return;
    }
    ...
    point_pairs.emplace_back(STPoint(start_s - adc_start_s, FLAGS_st_max_t),
                             STPoint(end_s - adc_start_s, FLAGS_st_max_t));// 时间从0一直到最大。因为是静态的
    reference_line_st_boundary_ = STBoundary(point_pairs);
  } else {
    if (BuildTrajectoryStBoundary(reference_line, adc_start_s, // 对计算本障碍物在本车道（不是指障碍物所在的车道，对一个有关的障碍物，每条车道都会有一个obstacle对象！）上计算占据的时间和距离范围。
...
bool Obstacle::BuildTrajectoryStBoundary(const ReferenceLine& reference_line,
  ...
  // 对计算本障碍物在本车道（不是指障碍物所在的车道，对一个有关的障碍物，每条车道都会有一个obstacle对象！）上计算占据的时间和距离范围。
  ...
  const auto& trajectory_points = trajectory_.trajectory_point(); //trajectory_是预测轨迹，格式：
  // message Trajectory {
  // optional double probability = 1;  // probability of this trajectory
  // repeated apollo.common.TrajectoryPoint trajectory_point = 2;
  // 其中TrajectoryPoint包含位置、速度、加速度、转向、相对时间等信息。
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    double object_moving_box_length = // 前一个轨迹点到本轨迹点的距离+障碍物本身的长度。
    ...
    //从low_s开始判断，如果车在这里，是不是会与这一时刻的障碍物发生碰撞。
    //这个循环，其实是对时间增加的循环。
    bool has_low = false;
    double high_s =
        std::min(object_boundary.end_s() + adc_half_length, FLAGS_st_max_s);
    bool has_high = false;
    while (low_s + st_boundary_delta_s < high_s && !(has_low && has_high)) {//has_low has_high至少有一个为false。
      //如果两个都是true，说明找到了本时刻障碍物占据的s范围，即lows到highs。
      if (!has_low) {
  ...
  if (!polygon_points.empty()) {
    // 排序，并且去除掉时间间隔太小的点。polygon_points一个按照时间升序排列的vector，元素是一个pair，指明某个时刻占据的s上下界。
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line) {
  if (!IsStatic()) { // 注意只有静态障碍物才会设置lane_blocking
  ...
  if (sl_boundary_.start_l() * sl_boundary_.end_l() < 0.0) { // 横跨两侧
