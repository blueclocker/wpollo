...
Obstacle* ReferenceLineInfo::AddObstacle(const Obstacle* obstacle) {
  ...
  // path_decision_包含了障碍物信息。 将obstacle插入到path_decision_的obstacles_成员，就是一个map，是index到obstacle的映射。
  // 返回的mutable_obstacle是插入之后的障碍物的指针。
  // 每条车道都有自己的path_decision_，往里面插入障碍物的时候，会发生复制。也就是说实际上的同一个障碍物，在不同的
  // 参考线的path_decision_里是不同的对象。所以后面对障碍物计算sl boundary的时候，要记得其实这里的障碍物是附属于车道线的。
  ...
  // 将obstacle的顶点转为sl坐标，插入到perception_sl的boundary_point中；并且计算s和l的范围，赋值给相应的成员。
  ...
  mutable_obstacle->SetPerceptionSlBoundary(perception_sl); // 就是赋值。
  mutable_obstacle->CheckLaneBlocking(reference_line_); // 注意只有静态障碍物才会设置lane_blocking
...
// 判断是否要考虑这个障碍物：本车所在车道之外的车道上的障碍物都要考虑（除了离得特别远的）；本车所在车道上的障碍物，只考虑车前方一段距离的障碍物。
...
bool ReferenceLineInfo::IsIrrelevantObstacle(const Obstacle& obstacle) {
  ...
  // 如果本车在当前车道，并且障碍物不在车前方，并且障碍物在本车道，忽略。
  ...
  // 没忽略的障碍物：本车所在车道之外的车道上的障碍物都要考虑（除了离得特别远的）；本车所在车道上的障碍物，只考虑车前方一段距离的障碍物。
  return false;
}
