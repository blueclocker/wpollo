...
void ReferencePathImpl::setOriginalSpline(const PathBoostNS::tk::spline &x_s,
...
const std::vector<BState> &ReferencePathImpl::getReferenceStates() const {
...
BState ReferencePathImpl::getApproxState(const BState &original_state, const BState &actual_state, double len) const {
    // Point on reference.
...
void ReferencePathImpl::updateBoundsOnInputStates(const Map &map, std::vector<SLState> &input_sl_states) {
    ...
    for (size_t i = 0; i < input_sl_states.size(); ++i) {
        const BState& ref_state = reference_states_.at(i);
        ...
        BState front_center(ref_state.x + front_length_new * cos(ref_state.heading),
                           ref_state.y + front_length_new * sin(ref_state.heading),
                           ref_state.heading);
        BState rear_center(ref_state.x + rear_length_new * cos(ref_state.heading),
...
void ReferencePathImpl::updateBoundsImproved(const PathBoostNS::Map &map) {
    if (reference_states_.empty()) {
        LOG(WARNING) << "Empty reference, updateBounds fail!";
        return;
    }
    ...
    for (const auto &state : reference_states_) {
        // Front and rear bounds.
        BState front_center(state.x + FLAGS_front_length * cos(state.heading),
                           state.y + FLAGS_front_length * sin(state.heading),
                           state.heading);
        BState rear_center(state.x + FLAGS_rear_length * cos(state.heading),
...
std::vector<double> ReferencePathImpl::getClearanceWithDirectionStrict(const PathBoostNS::BState &state,
...
bool ReferencePathImpl::buildReferenceFromStates(const std::vector<PathBoostNS::BState> &states) {
