 ...
 * @Date: 2022-08-27 15:49:44
 * @LastEditors: blueclocker 1456055290@hnu.edu.cn
 * @LastEditTime: 2022-08-28 20:04:43
 * @FilePath: /wpollo/src/lanelet/path_optimizer_2/src/test/osmplan.cpp
...
void referenceCb(const visualization_msgs::Marker::ConstPtr &p) {
    ...
    {
        reference_point.x = p->points[i].x;
        reference_point.y = p->points[i].y;
    }
...
void startCb(const osmmap::CarState::ConstPtr &start) {
    start_state.x = start->carPose.position.x;
    start_state.y = start->carPose.position.y;
    start_state.heading = tf::getYaw(start->carPose.orientation);
    if (reference_rcv) {
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber reference_sub =
        nh.subscribe("/mapio/golbalpath_info", 1, referenceCb);
    ros::Subscriber start_sub =
        nh.subscribe("/mapio/carstate_info", 1, startCb);
