 ...
 * @Date: 2022-09-06 22:05:37
 * @LastEditors: blueclocker 1456055290@hnu.edu.cn
 * @LastEditTime: 2022-09-13 22:34:22
 * @FilePath: /wpollo/src/lanelet/hastar/src/hybidacore.cpp
...
{
    setVehicleShape(0.2, 0.1, 0.1);//setVehicleShape(4.7, 2.0, 1.3);
    ...
    std::cout << "current_costmap_ptr_.data.size: " << current_costmap_ptr_.data.size() << std::endl;
    std::cout << "STATE_GRID_SIZE_X Y: " << STATE_GRID_SIZE_X_ << " " << STATE_GRID_SIZE_Y_ << std::endl;
    ...
    std::cout << "MAP_GRID_SIZE_X Y: " << MAP_GRID_SIZE_X_ << " " << MAP_GRID_SIZE_Y_ << std::endl;
    ...
    for (unsigned int w = 0; w < map_w; ++w) 
    {
        for (unsigned int h = 0; h < map_h; ++h) 
        {
            ...
            // std::cout << "gridmap index: " << y * current_costmap_ptr_.info.width + x << std::endl;
...
bool hybidacore::search(const Vec3d &start, const Vec3d &goal)
{
    ...
    if(hasObstacle(start_grid_index.x(), start_grid_index.y())) return false;
    ...
    while(!openlist_.empty())
    {
        ...
        if ((current_node_ptr->pos.head(2) - goal_node_ptr->pos.head(2)).norm() <= shot_distance_) 
        {
            ...
            // std::cout << "before analyticExpansions" << std::endl;
            if (analyticExpansions(current_node_ptr, goal_node_ptr, rs_length)) 
            {
                ...
                // {
                //     std::cout << grid_node_ptr->pos << std::endl;
            ...
            std::cout << "RS shot fail" << std::endl;
        }

        std::cout << "before get neighbors" << std::endl;
        getNeighbors(current_node_ptr, neighbor_nodes_ptr);
        std::cout << "neighbor_nodes_ptr size is " << neighbor_nodes_ptr.size() << std::endl;
        ...
        std::cout << "finish a point !" << std::endl;
...
VectorVec3d hybidacore::getPath() const
{
    ...
    Vec3d prepos = state_grid_node_ptr->pos;//
    bool firstin = true;//
    while (state_grid_node_ptr != nullptr) {
        temp_nodes.emplace_back(state_grid_node_ptr);
        std::cout << "pos: " << state_grid_node_ptr->pos.transpose() << std::endl;
        if(!firstin && (prepos.head(2) - state_grid_node_ptr->pos.head(2)).norm() <= 1)//
        {
            std::cout << "break" << std::endl;
            break;//
        }//
        prepos = state_grid_node_ptr->pos;//
...
bool hybidacore::analyticExpansions(const HAstate::ptr &current_node_ptr, const HAstate::ptr &goal_node_ptr, double &length)
{
    ...
    // std::cout << "after detect pose, pose size is: " << rs_path_poses.size() << std::endl;
    ...
    // std::cout << "before return analyticExpansions" << std::endl;
...
void hybidacore::getNeighbors(const HAstate::ptr &current_node_ptr, std::vector<HAstate::ptr> &neighbor_nodes)
{
    ...
    for (int i = -steering_discrete_num_; i <= steering_discrete_num_; ++i)
    {
        ...
        {
            dynamicModel(-move_step_size_, phi, x, y, theta);
            // dynamicModel(-move_step_size_, phi+0.1, x, y, theta);//加0.1rad避免前后采样重复,没用
            intermediate_state.emplace_back(Vec3d(x, y, theta));
