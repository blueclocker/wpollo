#include "../include/pcl_test/pcl_test_core.h"

PclTestCore::PclTestCore(ros::NodeHandle &nh) //
{
    sub_point_cloud_ = nh.subscribe("/rslidar_points", 1, &PclTestCore::point_cb, this); // velodyne_points  add_points
    ...
    pub_point_ = nh.advertise<sensor_msgs::PointCloud2>("/relidar_points", 1);
    ros::spin();
}
...
void PclTestCore::clip_above(double clip_height, const pcl::PointCloud<pcl::PointXYZ>::Ptr in,
                             const pcl::PointCloud<pcl::PointXYZ>::Ptr out)
{
    // 创建矩阵对象_3阶float类型的矩阵  雷达标定
    Eigen::Matrix<double, 4, 4> twist;
    /*twist << 0.9999782, 1.063283e-05, -0.006593098, 0,
              1.063184e-05, 0.9999948, 0.003225248, 0,
              0.006593098, -0.003225248, 0.9999731,0,
               0, 0, 0, 1;*/
    // cs75

    twist << 0.9996844, 0.0001941956, -0.02512014, 0,
        0.0001941398, 0.9998806, 0.01545577, 0,
        0.02512014, -0.01545577, 0.9995649, 0,
        0, 0, 0, 1; // ev200
    /*

      Eigen::Affine3f transform_2=Eigen::Affine3f::Identity();//transform_2具有平移和旋转两个分向量
      transform_2.translation()<<0,0,0;//定义平移x,y,z三方向平移
      float theta_x=0.0;    //旋转弧度
      float theta_y=0.0;    //旋转弧度
      float theta_z=0.0;    //旋转弧度 右手坐标系，角度为正  /4
      transform_2.rotate(Eigen::AngleAxisf(theta_x,Eigen::Vector3f::UnitX()));//同理，UnitX()，绕x轴；UnitY()，绕y轴
      transform_2.rotate(Eigen::AngleAxisf(theta_y,Eigen::Vector3f::UnitY()));//同理，UnitX()，绕x轴；UnitY()，绕y轴
      transform_2.rotate(Eigen::AngleAxisf(theta_z,Eigen::Vector3f::UnitZ()));//同理，UnitX()，绕x轴；UnitY()，绕y轴
      // 执行转换
      pcl::PointCloud<pcl::PointXYZ>::Ptr in_out(new pcl::PointCloud<pcl::PointXYZ>);
      pcl::transformPointCloud(*in,*in_out,twist);//transform_2

     */

    pcl::ExtractIndices<pcl::PointXYZ> cliper;

    cliper.setInputCloud(in); // in原始点云输入  in_out是旋转变换
    pcl::PointIndices indices;

    for (size_t i = 0; i < in->points.size(); i++)
    {
        if ((in->points[i].x * in->points[i].x + in->points[i].y * in->points[i].y) < 1) // in->points[i].z < clip_height  &&  不截取低于雷达clip_height距离
        {
...
void PclTestCore::remove_close_pt(double min_distance, const pcl::PointCloud<pcl::PointXYZ>::Ptr in,
                                  const pcl::PointCloud<pcl::PointXYZ>::Ptr out)
{
    pcl::ExtractIndices<pcl::PointXYZ> cliper;
    ...
    for (size_t i = 0; i < in->points.size(); i++)
    {
        ...
        if (in->points[i].x < -1.5 || fabs(in->points[i].y) > 1.0 || in->points[i].x > 2.5) // cs75 y>1 x>2.5 割除车上点 ev200用
        {
            indices.indices.push_back(i);
        }
    }
 ...
 */
void PclTestCore::XYZ_to_RTZColor(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud,
 ...
 */
void PclTestCore::classify_pc(std::vector<PointCloudXYZRTColor> &in_radial_ordered_clouds,
...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    XYZ_to_RTZColor(remove_close, organized_points,
    ...
    publish_cloud(pub_point_, cliped_pc_ptr, in_cloud_ptr->header);

    /* ***       计时结束   * * ** **/
    clock_t end = clock();
    ROS_INFO("Running time: %f", ((double)(end) - (double)(begin)) / CLOCKS_PER_SEC * 1000);
}
