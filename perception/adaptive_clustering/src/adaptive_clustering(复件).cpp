...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) 
{
  ...
  for(int i = 0; i < pc_indices->size(); i++)//开始历变滤波后的点云
	...
	pcl_pc_in->points[(*pc_indices)[i]].z * pcl_pc_in->points[(*pc_indices)[i]].z;//求点的x*x+y*y+z*z之和给d2
  ...
  for(int i = 0; i < region_max_; i++) 
 {
    ...
    if(indices_array[i].size() > cluster_size_min_) 
      ...
      tree->setInputCloud(pcl_pc_in, indices_array_ptr);//indices_array_ptr每片区域内点云指针
  ...
  if(cloud_filtered_pub_.getNumSubscribers() > 0) {
    ...
    pcl::copyPointCloud(*pcl_pc_in, *pc_indices, *pcl_pc_out);//把pcl_pc_in中点和pc_indices下表复制给pcl_pc_out
...
int main(int argc, char **argv) 
{
  ...
  /*** Subscribers ***/
  ros::NodeHandle nh;
  ros::Subscriber point_cloud_sub = nh.subscribe<sensor_msgs::PointCloud2>("rslidar_points", 10, pointCloudCallback);//velodyne  rslidar   cloud_add_points
...
marker_array_pub_ = private_nh.advertise<visualization_msgs::MarkerArray>("markers_tubao", 100);
  ...
  private_nh.param<float>("z_axis_min", z_axis_min_, -1.4);
  private_nh.param<float>("z_axis_max", z_axis_max_, 05);
