...
{
  if(print_fps_)if(reset){frames=0;start_time=clock();reset=false;}//fps  ros中点云ros_pc2_in  函数名字 pointCloudCallback
  
  /*** Convert ROS message to PCL ***/
  pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_pc_in(new pcl::PointCloud<pcl::PointXYZI>);//定义一个pcl点云指针pcl_pc_in
  pcl::PointCloud<pcl::PointXYZI>::Ptr out(new pcl::PointCloud<pcl::PointXYZI>);//定义一个pcl点云指针pcl_pc_in
  pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_region(new pcl::PointCloud<pcl::PointXYZI>);//定义一个pcl点云指针pcl_pc_in
...
#pragma omp for
 ...
 pcl::CropBox<pcl::PointXYZI> region(true);
	...
	region.setMax(Eigen::Vector4f( 30, 10, 0, 1.0));//30, 7.5, 1.2, 1       30, 5, 0, 1.0
  ...
  pcl::PassThrough<pcl::PointXYZI> pt;//直通滤波
  pt.setInputCloud(out);//pcl_pc_in
  pt.setFilterFieldName("z");
  pt.setFilterLimits(z_axis_min_, z_axis_max_);//-0.8到2米
...
/* */
  ...
  float tolerance = 0.0;
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr > > clusters;//定义了两个模班类 vector<数据类型> 名称 前一个是容器，后一个是内存分配，Allocator 负责提供 vector 需要用到的动态内存 点云指针的vector
  ...
  for(int i = 0; i < region_max_; i++) 
 {
    ...
    if(indices_array[i].size() > cluster_size_min_) 
      ...
      {
      	pcl::PointCloud<pcl::PointXYZI>::Ptr cluster(new pcl::PointCloud<pcl::PointXYZI>);//定义聚类后
  ...
  if(cloud_filtered_pub_.getNumSubscribers() > 0) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_pc_out(new pcl::PointCloud<pcl::PointXYZI>);//定义输出点云集pcl_pc_out
...
/*     3米的车道内的包围盒   */
    if(marker_array_pub_.getNumSubscribers() > 0 || msg_array_pub_.getNumSubscribers() > 0||box_array_pub_.getNumSubscribers() > 0 ) 
      {
      Eigen::Vector4f min, max;
      pcl::getMinMax3D(*clusters[i], min, max);
   /* 凸包  */ 
...
msg_array.polygon_point.clear();
     ...
     visualization_msgs::Marker marker;
      ...
      for(int i = 0; i <hull.size()*2; i=i+2)  
         {
      p[i].x = hull[i/2].x;  p[i].y = hull[i/2].y;  p[i].z = max[2];
      p[i+1].x =hull[i/2].x; p[i+1].y = hull[i/2].y;  p[i+1].z = min[2];
...
int main(int argc, char **argv) 
{
  ...
  /*** Subscribers ***/
  ros::NodeHandle nh;
  ros::Subscriber point_cloud_sub = nh.subscribe<sensor_msgs::PointCloud2>("filtered_points_no_ground", 10, pointCloudCallback);//velodyne  rslidar   cloud_add_points
  ...
  msg_array_pub_ = private_nh.advertise<rs_perception::PerceptionListMsg>("rs_percept_result", 100);//发送顶点坐标
box_array_pub_ = private_nh.advertise<visualization_msgs::MarkerArray>("markers_tubao", 100);//包围盒
marker_array_pub_ = private_nh.advertise<visualization_msgs::MarkerArray>("markers_line", 100);//包围盒
  /*** Parameters ***/
  ...
  private_nh.param<int>("cluster_size_min", cluster_size_min_, 15);
  ...
  if(sensor_model.compare("VLP-16") == 0) {
    ...
    regions_[5] = 3; regions_[6] = 3; regions_[7] = 2; regions_[8] = 
3; regions_[9] = 3;
    regions_[10]= 3; regions_[11]= 3; regions_[12]= 3; regions_[13]= 3;
  } else if (sensor_model.compare("HDL-32E") == 0) {
    regions_[0] = 100; regions_[1] = 10; regions_[2] = 10; regions_[3] = 10; regions_[4] = 10;
    regions_[5] = 10; regions_[6] = 10; regions_[7] = 10; regions_[8] =10; regions_[9] = 10;
    regions_[10]= 5; regions_[11]= 5; regions_[12]= 4; regions_[13]= 5;//4 5  4 5 4 5 5 4 5 4
