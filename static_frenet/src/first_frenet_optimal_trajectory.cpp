...
Vec_Path calc_frenet_paths(
  ...
  for(float di=-1*MAX_ROAD_WIDTH; di<MAX_ROAD_WIDTH; di+=D_ROAD_W){ // MAX_ROAD_WIDTH  7.0   D_ROAD_W  1.0
    for(float Ti=MINT; Ti<MAXT; Ti+=DT){ // MAXT  5.0   MINT  4.0   DT  0.2
      FrenetPath fp;
      ...
      for(float tv=TARGET_SPEED - D_T_S * N_S_SAMPLE; //TARGET_SPEED  30.0 / 3.6   D_T_S  5.0 / 3.6  N_S_SAMPLE  1
...
int main(){
  ...
  for(int i=0; i<SIM_LOOP; i++){  //SIM_LOOP 500
    ...
    // visualization
    cv::Mat bg(1000, 4000, CV_8UC3, cv::Scalar(255, 255, 255));
